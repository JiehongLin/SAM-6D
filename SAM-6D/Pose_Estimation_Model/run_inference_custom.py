...
def get_parser():
    ...
    parser.add_argument('--template_dir', type=str, default='templates', help='path to templates')
...
def get_test_data(rgb_path, depth_path, cam_path, cad_path, seg_path, det_score_thresh, cfg, gt_segm=False):
    dilation_flag = False
    dets = []
    with open(seg_path) as f:
        dets_ = json.load(f) # keys: scene_id, image_id, category_id, bbox, score, segmentation
    if dets_:
        sorted_dets = sorted(dets_, key=lambda det: det['score'], reverse=True)
        best_dets = sorted_dets[:1] # take the best detection
        dets.extend(best_dets)
    del dets_

    cam_info = json.load(open(cam_path))
    K = np.array(cam_info['cam_K']).reshape(3, 3)
    # cam_info['depth_scale'] = 1000.0
    ...
    #plot the whole depth over the image with matplotlib
    # import matplotlib.pyplot as plt
    # plt.imshow(whole_image)
    # plt.imshow(whole_depth, alpha=0.5)
    # plt.show()
    
    # print('whole_depth min max', np.min(whole_depth), np.max(whole_depth))
    ...
    for inst in dets:
        ...
        if gt_segm:
            # Use bbox directly and enlarge by 10%
            x, y, w_box, h_box = inst['bbox']
            ...
            new_x2 = int(min(whole_image.shape[1], center_x + new_w / 2))
            new_y1 = int(max(0, center_y - new_h / 2))
            new_y2 = int(min(whole_image.shape[0], center_y + new_h / 2))

            # Create a mask using the enlarged bbox
            mask = np.zeros((whole_image.shape[0], whole_image.shape[1]), dtype=np.uint8)
            mask[new_y1:new_y2, new_x1:new_x2] = 1
            # show the mask over the image with matplotlib
            # import matplotlib.pyplot as plt
            # plt.imshow(whole_image)
            ...
            y1, y2, x1, x2 = new_y1, new_y2, new_x1, new_x2
            

        else:
            # Use original segmentation mask
            ...
            if dilation_flag:
                # print("dilation")
                # Apply dilation to expand the segmentation
                kernel = np.ones((5, 5), np.uint8)  # Bigger kernel → more dilation
        ...
        #plot the mask over the image with matplotlib
        # import matplotlib.pyplot as plt
        # plt.imshow(whole_image[y1:y2, x1:x2, :])
        # plt.imshow(mask, alpha=0.5)
        # plt.show()
        ...
        # # add reference origin
        # lrf = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.1, origin=[0, 0, 0])
        # o3d.visualization.draw_geometries([pcd,lrf ], window_name=f"PCD from detection {inst['category_id']}")
...
if __name__ == "__main__":
    ...
    filename = os.path.basename(cfg.rgb_path).replace(".png", "_pem")
    ...
    with torch.no_grad():
        ...
        print(input_data['dense_fo'].size())
        print(input_data['dense_po'].size())
    ...
    with open(os.path.join(f"{cfg.output_dir}/sam6d_results", f'{filename}.json'), "w") as f:
        json.dump(detections, f)

    print("=> visualizating ...")
    save_path = os.path.join(f"{cfg.output_dir}/sam6d_results", f'{filename}.png')
